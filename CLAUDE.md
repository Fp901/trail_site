...
- **`BookingWidget`** (island) — see Part 9 and **§17.0**. Takes a `catering` **prop** (`'catered'` on `/rates`, `'uncatered'` on the unlisted page): catering is never asked of the guest. **Four steps:** (1) your group: walkers 2 to 8, with a "Travelling solo?" enquiry panel; "How many rooms does your group need?" (a stepper in steps of 1, from all doubles up to one each, never past 4; the own-room count is derived as 2 x rooms - walkers, and a note names who is alone and the supplement); "Does anyone in your group live in an SADC country?" (No preselected, with the 16 countries behind a disclosure; only Yes reveals the 30% discount, a count from 1 and the SADC tick); the age tick; (2) start date; (3) details; (4) review and pay. The self-catered mount fixes 8 SADC guests sharing 4 rooms. Price lines: "All-inclusive rate, n x R", "SADC resident rate (30% off), n x R", "Single supplement, n x R", the last-minute line, the total. Showing the 30% off label is intended. **"Self-catered" appears only in the widget on the hidden page**; the public mount never names it, and a date held by the other product is drawn and announced as plain "unavailable" (three legend keys: open, guaranteed departure, unavailable).
...
- Intro: one price per person for the whole 3-night trail, VAT and levies included, from 2 guests up, varying by season. Beneath the rates table: "A Single Supplement of 40% of the All-inclusive rate applies to all bookings regardless of SADC residency." Plain, one fact per sentence: a child should be able to follow how to book.
...
### 9.3 Data &amp; libs
- Schema + RLS: `supabase/migrations/` applied in filename order (bookings, inquiries, blocked_dates, pretrip_details, payment_events, admin_audit, rate_limits; the anon-readable `departure_inventory` view; **RLS default-deny**; the `bookings_slot_guard`/`bookings_window_guard` triggers). **0019 is the current head**: `departure_inventory` becomes a SECURITY INVOKER view over `private.departure_inventory_rows()`, a SECURITY DEFINER function in the unexposed `private` schema with an empty `search_path` (clears Supabase's "Security Definer View" finding; same columns, same client). 0018 put the slot guard's joining minimum back to 2. 0017 adds `single_rooms`, `sadc_count`, `age_confirmed_at` and the generated `rooms` column, caps each date at 4 rooms and 8 walkers in the slot guard, and keys the 12-month window on `sadc_count > 0` (§17.0). 0016 dropped the `bookings_unique_start_date` index.
...
19. **Rooms, single supplement and SADC count (23 September 2026).** Capacity moved from 8 places to 4 double rooms (still 8 walkers); guests share only within their own booking; a 40% single supplement per own room; solo walkers may join a guaranteed departure; the country question is replaced by an SADC count, with the discount shown openly in the widget; the SADC-proof premium is 50% on both products (was 100% / "international rates" on the hidden page); the widget goes from 5 steps to 4. Migration 0017. **§17.0 specifies it.**

---

## Part 17 — Commercial model v4 (canonical specification)

### 17.0 Rooms model (23 September 2026) — overrides 17.1 to 17.8 where they differ

- **Capacity:** 4 double rooms per departure, at most 8 walkers (`ROOMS_PER_DEPARTURE`, `MAX_GROUP_SIZE`). Guests only share a room within their own booking. Rooms used = `singleRooms + (groupSize - singleRooms) / 2`; the number sharing must be even.
- **Party:** catered 2 to 8. Opening an empty date takes 2 (any room mix), and joining a booked date also takes 2 (25 September 2026, migration 0018): solo walkers are not booked online, and the widget's "Travelling solo?" panel takes their details as an enquiry instead. Self-catered is exactly 8 SADC guests sharing 4 rooms. `booking_type = 'exclusive'` when one booking takes all 4 rooms (derived by the trigger).
