...
### 9.3 Data &amp; libs
- Schema + RLS: `supabase/migrations/` applied in filename order (bookings, inquiries, blocked_dates, pretrip_details, payment_events, admin_audit, rate_limits; the anon-readable `departure_inventory` view; **RLS default-deny**; the `bookings_slot_guard`/`bookings_window_guard` triggers). **0018 is the current head** (the slot guard's joining minimum back to 2). 0017 adds `single_rooms`, `sadc_count`, `age_confirmed_at` and the generated `rooms` column, caps each date at 4 rooms and 8 walkers in the slot guard, and keys the 12-month window on `sadc_count > 0` (§17.0). 0016 dropped the `bookings_unique_start_date` index.
