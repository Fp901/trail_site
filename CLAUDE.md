...
### 9.3 Data &amp; libs
- Schema + RLS: `supabase/migrations/` applied in filename order (bookings, inquiries, blocked_dates, pretrip_details, payment_events, admin_audit, rate_limits; the anon-readable `departure_inventory` view; **RLS default-deny**; the `bookings_slot_guard`/`bookings_window_guard` triggers). **0017 is the current head**: it adds `single_rooms`, `sadc_count`, `age_confirmed_at` and the generated `rooms` column, caps each date at 4 rooms and 8 walkers in the slot guard, and keys the 12-month window on `sadc_count > 0` (§17.0). 0016 dropped the `bookings_unique_start_date` index.
...
- **Party:** catered 1 to 8. Opening an empty date takes 2 (any room mix); joining a booked date takes 1, so a solo walker can only join a guaranteed departure. Self-catered is exactly 8 SADC guests sharing 4 rooms. `booking_type = 'exclusive'` when one booking takes all 4 rooms (derived by the trigger).
...
- **SADC proof:** a guest counted as SADC who cannot show ID or a passport at registration pays a **100% premium** (`SADC_PREMIUM_PCT`; 50% from 23 September, raised to 100% on 24 September 2026), on both products. Stated in the widget tick, the booking terms and the FAQ; never computed.
...
A product is `catering × residency`; there are no other columns. **International self-catered is not sold**: the hidden page states that a guest who cannot show SADC proof at check-in pays a 100% premium (§17.0), which is an on-the-day operator matter, not something the engine prices.
