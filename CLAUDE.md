...
- **The guest states counts, not a country.** The country question is gone. The widget asks how many need their own room and how many are SADC residents; `createCheckout` takes `{ startDate, catering, groupSize, singleRooms, sadcCount, residencyDeclaration, ageConfirmed, lead*, company }`. Never a price or a band. `getRateContext` is removed; public rates ship with the page for the estimate, and the server recomputes everything.
...
- **SADC proof:** a guest counted as SADC who cannot show ID or a passport at registration pays a **50% premium** (`SADC_PREMIUM_PCT`), on both products. Stated in the widget tick, the booking terms and the FAQ; never computed.
