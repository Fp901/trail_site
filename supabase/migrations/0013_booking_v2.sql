-- The Rooiberg Wander — Booking v2: catered/uncatered pricing + shared Sunday/Monday departures
--
-- WHY (operator user stories, v2.2 revision):
...
--   2. SUNDAY and MONDAY become SHARED/MIXED, catered-only departure days, each taking
--      MULTIPLE group bookings: each booking 2..8 people, 8 places total across bookings
--      PER DATE, priced per person per night. Tuesday to Saturday remain private/exclusive
--      (one private group per start date), self-catered up to 10 guests or catered up to 8.
...
--   exclusive bookings: unique start_date among active rows (partial unique index), Tuesday
--     to Saturday only (trigger); self-catered group_size <= 10, catered group_size <= 8.
--   shared bookings: Sunday or Monday only; multiple active rows may share a start_date;
--     total seats (sum of group_size) capped at 8 per date by a trigger under an advisory
--     lock (the DB is the last line of defence against concurrent seat-grabs; the server
--     action checks first for UX).
...
comment on index public.bookings_unique_start_date is
  'One active EXCLUSIVE booking per start_date (staggered-groups model). Shared Sunday/Monday '
...
-- ---------------------------------------------------------------------------
-- 3. Slot guard trigger — Sunday and Monday are shared-only, Tuesday-Saturday are
--    exclusive-only; exclusive group size is capped by catering; shared capacity is 8 seats
--    per date. BEFORE INSERT OR UPDATE so admin date-moves are covered too.
-- ---------------------------------------------------------------------------
...
begin
  ...
  if new.booking_type = 'exclusive' then
    if v_dow in (7, 1) then
      raise exception 'RW_PRIVATE_TUE_SAT_ONLY: private bookings run Tuesday to Saturday only'
        using errcode = 'P0001';
    end if;
    if new.catering = 'catered' and new.group_size > 8 then
      raise exception 'RW_CATERED_MAX_8: catered private bookings take up to 8 guests'
        using errcode = 'P0001';
    end if;
    if new.catering = 'uncatered' and new.group_size > 10 then
      raise exception 'RW_UNCATERED_MAX_10: self-catered private bookings take up to 10 guests'
        using errcode = 'P0001';
    end if;
    return new;
  end if;

  -- Shared rows: Sunday or Monday only, 2..8 people per booking, 8 seats total per date.
  if v_dow not in (7, 1) then
    raise exception 'RW_SHARED_SUN_MON_ONLY: shared departures start on Sundays or Mondays only'
      using errcode = 'P0001';
  end if;
  ...
  -- Serialize concurrent seat-grabs for the same date (transaction-scoped advisory lock),
...
--    then, OR it falls in an operator-blocked window, OR it is a FULL shared Sunday/Monday (8
--    seats held). Partially booked shared dates stay bookable. Same shape/grants as 0009/0011.
-- ---------------------------------------------------------------------------
...
create view public.unavailable_windows as
  ...
  union
  -- A shared Sunday/Monday date is unavailable once fewer than SHARED_MIN_PEOPLE (2) seats
  -- remain, not only when fully booked — a new booking could not meet the 2-person minimum
  -- otherwise. Keep the literal "6" (= 8 capacity - 2 minimum) in sync with
  -- SHARED_MAX_CAPACITY/SHARED_MIN_PEOPLE in src/data/rates.ts.
  select start_date from public.bookings
    where booking_type = 'shared'
      and (status = 'confirmed' or (status = 'pending' and hold_expires_at > now()))
    group by start_date
...
comment on view public.unavailable_windows is
  ...
  'Sunday/Monday dates. Partially booked shared dates are NOT listed (they remain bookable; '
  'see shared_slot_availability for seats). Do not add PII columns here.';

-- ---------------------------------------------------------------------------
-- 5. Shared-seat availability view — the calendar shows places left per shared date. PII-free
--    aggregate; a shared date with no row simply has all 8 places free.
-- ---------------------------------------------------------------------------
...
comment on view public.shared_slot_availability is
  'Intentional SECURITY DEFINER gateway: seats remaining per shared Sunday/Monday date (no '
  'PII — only a date and a count). Dates absent from this view have all 8 places free. Do '
  'not add PII columns here.';
